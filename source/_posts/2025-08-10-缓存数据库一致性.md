...
事务的一致性是数据逻辑上的一致性，满足一定的逻辑约束。

缓存数据库的一致性是指的是多个缓存中的数据库副本要和数据库数据保持一致性。



不应该缓存查询频率低，频繁更新，数据强一致性极高的也不建议。
...
- 删除数据，如果缓存中有这个数据？问题就复杂。

  - 先删缓存再删数据库。不行，中间来了个读请求，打到数据库，不就会重新缓存了吗。
  - 先删数据库再删缓存。T1线程查询，T2线程执行删除操作。这时候就存在不一致性了。
  ...
  sequenceDiagram
      ...
      T1->>DB: query old value
      T2->>DB: delete
      T2->>Cache: delete
      T1->>Cache: set old value
...
- 非侵入业务逻辑：或者监听数据库更新binlog，然后删除缓存，可以重试。 运维成本和一致性链路拉长。
